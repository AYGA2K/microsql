...
TEST_CASE("serializeRow INTEGER writes correct bytes") {
  ...
  int64_t out;
  std::memcpy(&out, result.value().data() + 1, 8);
  CHECK(out == 42);
}
...
TEST_CASE("serializeRow NULL writes only the null flag") {
  Schema schema = {makeCol(DataType::INTEGER)};
  Row row = {nullptr};
  auto result = serializeRow(row, schema);
  REQUIRE(result.has_value());
...
TEST_CASE("serializeRow TEXT writes correct bytes") {
  ...
  CHECK(result.value()[0] == 0);
  uint16_t len;
  std::memcpy(&len, result.value().data() + 1, 2);
  CHECK(len == 5);
  CHECK(std::memcmp(result.value().data() + 3, "hello", 5) == 0);
}
...
TEST_CASE("serializeRow multi-column row lays out fields contiguously") {
  ...
  CHECK(result.value()[0] == 0);
  int64_t ival;
  std::memcpy(&ival, result.value().data() + 1, 8);
  CHECK(ival == 7);
  CHECK(result.value()[9] == 0);
  CHECK(result.value()[10] == 0);
}

TEST_CASE("serializeRow multi-column row mixes NULL and non-NULL fields") {
  Schema schema = {makeCol(DataType::INTEGER), makeCol(DataType::INTEGER)};
  Row row = {int64_t(7), nullptr};
  auto result = serializeRow(row, schema);
  REQUIRE(result.has_value());
  CHECK(result.value().size() == 1 + 8 + 1);
  CHECK(result.value()[0] == 0);
  int64_t ival;
  std::memcpy(&ival, result.value().data() + 1, 8);
  CHECK(ival == 7);
  CHECK(result.value()[9] == 1);
}
...
TEST_CASE("deserializeRow TEXT reads correct value") {
  Schema schema = {makeCol(DataType::TEXT, 5)};
  std::vector<uint8_t> buf = {0, 5, 0, 'h', 'e', 'l', 'l', 'o'};
  auto result = deserializeRow(buf.data(), schema);
  REQUIRE(result.has_value());
  CHECK(std::get<std::string>((*result)[0]) == "hello");
}

TEST_CASE("deserializeRow NULL flag produces nullptr value") {
  Schema schema = {makeCol(DataType::INTEGER)};
  std::vector<uint8_t> buf = {1};
  auto result = deserializeRow(buf.data(), schema);
  REQUIRE(result.has_value());
  CHECK(std::holds_alternative<std::nullptr_t>((*result)[0]));
}
...
TEST_CASE("roundtrip NULL value") {
  Schema schema = {makeCol(DataType::INTEGER)};
  Row original = {nullptr};
  auto serialized = serializeRow(original, schema);
  REQUIRE(serialized.has_value());
  auto result = deserializeRow(serialized.value().data(), schema);
  REQUIRE(result.has_value());
...
TEST_CASE("roundtrip mixed schema with a NULL column") {
  Schema schema = {
      makeCol(DataType::INTEGER),
      makeCol(DataType::TEXT, 4),
  };
  Row original = {int64_t(99), nullptr};
  auto serialized = serializeRow(original, schema);
  REQUIRE(serialized.has_value());
  auto result = deserializeRow(serialized.value().data(), schema);
  REQUIRE(result.has_value());
  ...
  CHECK(std::holds_alternative<std::nullptr_t>(row[1]));
}
